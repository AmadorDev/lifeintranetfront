...
//register docs
export async function sliderDocApi(FormData, logout) {
  try {
    const url = `${BASE_PATH}/managements/docs/sliders`;
    const params = {
      method: "POST",
    };
    params.body = FormData;
...
//get list banner
export async function getBannerApi(s, cate, logout = null) {
  try {
    const url = `${BASE_PATH}/managements/sliders/${s}?cate=${cate}`;
    const params = {
      method: "GET",
    };
    const result = await authFetch(url, params, logout);
