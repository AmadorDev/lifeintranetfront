...
export default function Gallery() {
  ...
  const handleFileInputChange = async (event) => {
    ...
    try {
      ...
      const allowedExtensions = ["jpg", "jpeg", "png", "image/png"];
  ...
  const getGallery = async () => {
    try {
      const resp = await getBannerApi("marketing","photos", logout);
      if (resp.status) {
      ...
      <div className="flex justify-between">
        <div className="">
          <p className="font-bold">
            GALERIA DE FOTOS <strong>({galleries?.length})</strong>
          </p>
        </div>
                ...
                <div className="w-full md:h-36 h-28 overflow-hidden rounded-lg relative block">
                  <div className="iframe-container">
                    ...
                    <div className="absolute bottom-0 left-0 right-0 flex justify-center p-2">
                      ...
                      >
                      <FiDownload size={15}/>  <span className="mx-1">DESCARGAR</span>
                      </a>
                    </div>
                  </div>
                </div>
