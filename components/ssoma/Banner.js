...
export default function Banner() {
  ...
  const onSubmit = async () => {
    ...
    console.log(url);
    console.log(image);
    if (name.trim().length === 0 || url.trim().length === 0 || image == "") {
      toast.error("titulo , link , imagen son obligatorios");
      return;
    }
    ...
    <div>
      ...
      >
        <div className="w-full h-full">
          ...
          <span>Medida banner : 1248px - 698px</span>
          <div className="line">
            <input
              className="line__input"
              id="url"
              ...
              {item.link && (
                <div className="absolute bottom-0 left-0 p-4">
                  <a
                    href={item.link}
                    target="_blank"
                    className="bg-pri text-white px-4 py-3 rounded"
                  >
                    Más información
                  </a>
                </div>
